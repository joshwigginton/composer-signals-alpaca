...
def setup_logging():
    ...
    """
    log_file_name = f"log_paper_{datetime.now().strftime('%Y%m%d')}.txt"
...
def calculate_target_investment(api, cash_weight):
    """
    Calculate the target investment amount based on the account's equity and a specified cash weight. Cash weight is % invested 
    For example 1.15 would be 115% of the account's equity. 0.90 would be 90% of the account's equity.
...
def get_target_allocations(symphony_url, service_account_file, symphony_to_trade):
    ...
    if not all(col in df.columns for col in expected_columns):
        logging.error(f"Missing expected columns in CSV: {expected_columns}")
        return {}
...
def main():
    ...
    logging.info(f"New Portfolio Allocations: {target_allocations}")
